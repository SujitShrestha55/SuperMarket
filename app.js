...
function renderPage(){
  ...
  const previousPage=contentForTransition?.dataset?.pageName || "";
  const pageOrder=["Dashboard","Point of Sale","Inventory","Sales","Reports","Settings"];
  const oldIndex=pageOrder.indexOf(previousPage), newIndex=pageOrder.indexOf(state.page);
  if(contentForTransition && previousPage && previousPage!==state.page){
    contentForTransition.classList.remove("pageSlideInLeft","pageSlideInRight");
    void contentForTransition.offsetWidth;
    contentForTransition.classList.add(newIndex>=oldIndex?"pageSlideInRight":"pageSlideInLeft");
    contentForTransition.addEventListener("animationend",()=>contentForTransition.classList.remove("pageSlideInLeft","pageSlideInRight"),{once:true});
  }
  if(contentForTransition) contentForTransition.dataset.pageName=state.page;
