...
function dashboardRecentSalesHtml(sales){
  ...
  return `<div class="recentSalesList">${sales.slice(0,6).map(s=>`
...
function renderDashboard(){
  ...
  return `${headHtml("DASHBOARD",`Good morning, ${currentUserName()} 👋`,"Here's what's happening with your store today.",
    ...
    <div class="two">
      <div class="panel"><div class="panelHead"><div><h2>Sales Overview</h2><p>Last 7 days revenue</p></div><button class="link" id="dashViewSales">View all</button></div>
...
function renderPOS(){
  ...
  return `<div class="pos"><div class="panel">
      <div class="scan">${icon("barcode",20)}<input id="barcodeInput" ${state.scanMode==="camera"?"":"autofocus"} placeholder="${state.scanMode==="camera"?"Or type/scan barcode here...":"Scan barcode here..."}" value="${esc(state.barcode)}"/><kbd>ENTER</kbd><button class="camBtn${state.scanMode==="camera"?" primary":""}" id="cameraScanBtn" title="Scan with camera">${icon("camera",17)}</button></div>
...
function renderSales(){
  return `${headHtml("SALES","Sales History","View, print and void completed bills.",
    ...
    <div class="panel tablePanel scrollPanel"><div class="panelHead"><div><h2>Past Bills</h2><p>${state.sales.length} saved transaction${state.sales.length===1?"":"s"}${isAdmin()?"":" · voiding is admin-only"}</p></div></div>${salesTableHtml(state.sales)}</div>`;
}
